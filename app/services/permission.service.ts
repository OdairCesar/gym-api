...
export default class PermissionService {
  ...
  async getGymsWithPermissions(personalId: number) {
    return await db
      .from('gympermissions')
      .join('gyms', 'gyms.id', 'gympermissions.gym_id')
      .where('gympermissions.personal_id', personalId)
      .where('gympermissions.is_active', true)
      .select('gyms.*', 'gympermissions.can_edit_diets', 'gympermissions.can_edit_trainings')
  }

  async getPersonalsWithPermissions(gymId: number) {
    return await db
      .from('gympermissions')
      .join('users', 'users.id', 'gympermissions.personal_id')
      .where('gympermissions.gym_id', gymId)
      .where('gympermissions.is_active', true)
      .select('users.*', 'gympermissions.can_edit_diets', 'gympermissions.can_edit_trainings')
  }
