...
export default class extends BaseSchema {
  protected tableName = 'gymsubscriptions'

  async up() {
    this.schema.createTable(this.tableName, (table) => {
      table.increments('id')
      table.integer('gym_id').unsigned().references('id').inTable('gyms').onDelete('CASCADE')
      table.integer('gymplan_id').unsigned().references('id').inTable('gymplans').notNullable()
      table.enum('status', ['active', 'cancelled', 'past_due']).defaultTo('active').notNullable()
