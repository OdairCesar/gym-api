...
export const getAllProducts = async (req: Request, res: Response) => {
  try {
    ...
    if (nome) query = { ...query, nome: { $regex: nome, $options: 'i' } }
    if (categoria)
      query = { ...query, categoria: { $regex: categoria, $options: 'i' } }
    if (codigo) query = { ...query, codigo: { $regex: codigo, $options: 'i' } }
    if (ativo !== undefined) query = { ...query, ativo: ativo === 'true' }
